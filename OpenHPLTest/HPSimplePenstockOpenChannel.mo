...
model HPSimplePenstockOpenChannel "testing open channel"
  ...
  OpenHPL.Waterway.Pipe discharge(L=600, H=-5) annotation (Placement(transformation(extent={{6,0},{26,20}})));
  ...
  OpenHPL.Waterway.Pipe penstock(
    ...
    L=600) annotation (Placement(transformation(
        ...
        extent={{-10,-10},{10,10}},
        rotation=-90)));
  OpenHPL.ElectroMech.Turbines.Turbine turbine(C_v=3.7) annotation (Placement(transformation(origin={-12,14}, extent={{-10,-10},{10,10}})));
  inner OpenHPL.Data data(Vdot_0=18.9979) annotation (Placement(transformation(
        origin={-90,92},
        extent={{-10,-10},{10,10}})));
  OpenHPL.Waterway.Pipe pipe annotation (Placement(transformation(extent={{-74,34},{-54,54}})));
  OpenHPL.Waterway.OpenChannel openChannel(
    N=100,
    W=50,
    h_0=vector([ones(68)*0.30417; 0.3045; 0.3089; 0.33; 0.398; linspace(0.519, 5.00164, 28)])) annotation (Placement(transformation(extent={{30,2},{50,22}})));
  OpenHPL.Waterway.Pipe pipe1(H=0, L=10) annotation (Placement(transformation(extent={{54,-2},{74,18}})));
  OpenHPL.Waterway.SurgeTank surgeTank(h_0=73.9171) annotation (Placement(transformation(extent={{-52,38},{-32,58}})));
equation
  ...
  connect(discharge.n, openChannel.p)
    annotation (Line(points={{26,10},{28,10},{28,12},{30,12}},
                                               color={28,108,200}));
  connect(openChannel.n, pipe1.p) annotation (Line(points={{50,12},{52,12},{52,
          8},{54,8}},
                    color={28,108,200}));
...
end HPSimplePenstockOpenChannel;
