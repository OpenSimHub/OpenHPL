...
model HPSimpleElasticPenstockWithoutSurge "Model of HP system without surge tank and with elastic penctock (StagardGrid), but simplified models for turbine, etc."
  ...
  inner OpenHPL.Data data annotation (Placement(transformation(
        origin={-90,92},
        extent={{-10,-10},{10,10}})));
  OpenHPL.Waterway.Pipe intake(Vdot_0=20.6) annotation (Placement(transformation(extent={{-58,52},{-38,72}})));
  ...
  OpenHPL.Waterway.Reservoir tail(h_0=10) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={92,42})));
  OpenHPL.Waterway.Penstock penstock1(N=10, Vdot_0=20.6) annotation (Placement(transformation(origin={-6,48}, extent={{-10,-10},{10,10}})));
  OpenHPL.ElectroMech.Turbines.Turbine turbine1(WaterCompress=true) annotation (Placement(transformation(origin={24,36}, extent={{-10,-10},{10,10}})));
