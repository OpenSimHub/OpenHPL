...
model HPSimplePenstockFrancis2 "HP system model with Francis turbine and generator"
  ...
  OpenHPL.Waterway.Pipe discharge(L=600, H=0.5) annotation (Placement(transformation(extent={{54,30},{74,50}})));
  OpenHPL.Waterway.Reservoir tail(h_0=5) annotation (Placement(transformation(
        origin={94,44},
        extent={{-10,10},{10,-10}},
        rotation=180)));
  OpenHPL.ElectroMech.Generators.SynchGen generator(P_op=100e6, UseFrequencyOutput=false) annotation (Placement(transformation(extent={{16,-4},{40,20}})));
  ...
  inner OpenHPL.Data data(Vdot_0=4.54) annotation (Placement(transformation(
        origin={-90,92},
        extent={{-10,-10},{10,10}})));
  OpenHPL.Waterway.Fitting fitting(D_1=3, D_2=1.63) annotation (Placement(transformation(extent={{-4,20},{16,40}})));
equation
