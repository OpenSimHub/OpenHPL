...
model HPElasticKPPenstockHalfSurgeD "Similar to previous HP system, but with twice reduced surge tank diameter"
  ...
  OpenHPL.Waterway.Pipe intake(Vdot_0=19) annotation (Placement(transformation(extent={{-74,56},{-54,76}})));
  ...
  OpenHPL.Waterway.Reservoir tail(h_0=10) annotation (Placement(transformation(
        ...
        extent={{-10,10},{10,-10}},
        rotation=180)));
  OpenHPL.ElectroMech.Generators.SimpleGen unit(w_0=52) annotation (Placement(transformation(extent={{8,2},{28,22}})));
  OpenHPL.ElectroMech.Turbines.Turbine turbine(C_v=3.7, WaterCompress=true) annotation (Placement(transformation(extent={{8,32},{28,52}})));
  OpenHPL.Waterway.PenstockKP penstockKP(N=20, Vdot_0=19*ones(20)) annotation (Placement(transformation(extent={{-18,42},{2,62}})));
  OpenHPL.Waterway.SurgeTank surgeTank(D=1.7) annotation (Placement(transformation(extent={{-44,60},{-24,80}})));
equation
