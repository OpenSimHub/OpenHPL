...
model HPSimplePenstockResKP "Model of HP system with using reservoir model based on open channel"
  ...
  OpenHPL.Waterway.Pipe discharge(H=5, L=600) annotation (Placement(transformation(extent={{46,26},{66,46}})));
  OpenHPL.Waterway.Reservoir tail(h_0=10) annotation (Placement(transformation(
        origin={92,42},
        extent={{-10,10},{10,-10}},
        rotation=180)));
  OpenHPL.ElectroMech.Generators.SimpleGen unit annotation (Placement(transformation(extent={{18,-4},{38,16}})));
  ...
  OpenHPL.ElectroMech.Turbines.Turbine turbine(C_v=3.7) annotation (Placement(transformation(origin={28,36}, extent={{-10,-10},{10,10}})));
  inner OpenHPL.Data data annotation (Placement(transformation(
        origin={-90,92},
        extent={{-10,-10},{10,10}})));
  OpenHPL.Waterway.ReservoirChannel reservoir(
    N=20,
    W=1000,
