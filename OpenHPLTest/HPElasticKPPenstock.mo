...
model HPElasticKPPenstock "Model of HP system with elastic penctock (KP), but simplified models for turbine, etc."
  ...
  inner OpenHPL.Data data(Vdot_0=19.12, rho(displayUnit="kg/m3") = 997) annotation (Placement(transformation(
        origin={-90,90},
        extent={{-10,-10},{10,10}})));
  OpenHPL.Waterway.Pipe intake(H=23) annotation (Placement(transformation(extent={{-72,54},{-52,74}})));
  OpenHPL.Waterway.Pipe discharge(H=0.5, L=600) annotation (Placement(transformation(extent={{38,30},{58,50}})));
  ...
  OpenHPL.Waterway.SurgeTank surgeTank(h_0=69.9) annotation (Placement(transformation(extent={{-42,60},{-22,80}})));
  ...
  OpenHPL.ElectroMech.Generators.SimpleGen unit annotation (Placement(transformation(extent={{8,0},{28,20}})));
equation
  //19.077 * ones(10)
