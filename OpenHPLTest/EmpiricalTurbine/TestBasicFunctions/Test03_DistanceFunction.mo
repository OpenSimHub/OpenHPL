...
public
  parameter Real opening=0.67;
  Real cPoints[NP, ND];
  Real[ND] curvePoint;
  Real distance;
equation
  cPoints = OpenHPL.Functions.WeightedControlPoints(opening, tc);
  curvePoint = OpenHPL.Functions.deCasteljau(time, ND, cPoints);
  distance=sqrt((1.1-curvePoint[1])^2);
  annotation (
