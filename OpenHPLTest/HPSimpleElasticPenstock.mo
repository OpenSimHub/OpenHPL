...
model HPSimpleElasticPenstock "Model of HP system with elastic penctock (StagardGrid), but simplified models for turbine, etc."
  ...
  OpenHPL.Waterway.Reservoir tail(h_0=10) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={92,42})));
  OpenHPL.Waterway.SurgeTank surgeTank1(SteadyState=false, h_0=71) annotation (Placement(transformation(origin={-26,62}, extent={{-10,-10},{10,10}})));
  OpenHPL.Waterway.Penstock penstock(
    D_i=3,
    D_o=3,
    H=428.5,
    N=10,
    Vdot_0=19.06) annotation (Placement(transformation(origin={0,54}, extent={{-10,-10},{10,10}})));
