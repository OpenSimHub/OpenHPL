...
model KPOpenChannel "Implementation of the KP functions for an open channel"
  ...
  parameter SI.Length w = 194 "Channel width", L = 5000 "Channel length";
  parameter SI.Height H[2] = {16.7, 0} "Channel height, left and right side", b[N + 1] = linspace(H[1], H[2], N + 1) "Riverbed", h0[N] = vector([ones(5) * 0.4; linspace(H[1] - 0.4 - 0.5 * (b[6] + b[7]), H[1] - 0.4 - 0.5* (b[N] + b[N + 1]), N - 5)]) "Initial depth";
  ...
  input Real boundaryValues[2, 2] = [h0[1] + b[1],     Vdot_0 / w;
                                     h0[N] + b[N + 1], Vdot_0 / w] "Values for the boundary conditions [z_left, q_left; z_right, q_right]";
  SI.Length dx = L / N;
  ...
  SI.Velocity u_[N, 4];
  Real q0 = Vdot_0 / w;
  Real q[N](start = ones(N) * Vdot_0 / w);
  Real q_[N, 4];
  Real q_t;
  Real S_[2 * N];
  Real theta = 1.3;
