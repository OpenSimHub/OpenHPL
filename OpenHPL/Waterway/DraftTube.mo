...
model DraftTube "Model of a draft tube for reaction turbines"
  ...
  SI.VolumeFlowRate Vdot(start = Vdot_0, fixed = true) "Volumeteric flow rate";
  SI.VolumeFlowRate Vdot_b "Volumeteric flow rate for Branch section of Moody spreading pipes";

  Real cos_theta = Modelica.Math.cos(SI.Conversions.from_deg(theta))              "Calculating cos_theta";
  Real cos_theta_moody = Modelica.Math.cos(SI.Conversions.from_deg(theta_moody))              "Calculating cos_theta_moody";


  Real cos_theta_moody_by_2 = Modelica.Math.cos(SI.Conversions.from_deg(theta_moody/2))       "Calculating cos_theta_moody_by_2";
...
equation
  ...
  if DraftTubeType == OpenHPL.Types.DraftTube.ConicalDiffuser then
    ...
    V = C.pi*H/12*(D_i^2+D_o^2+D_i*D_o) "Volume of water inside the draft tube";
    v = Vdot/A_av;
