...
model Penstock "Model of the penstock with elastic walls and compressible water. Simple Staggered grid scheme"
  ...
  SI.Area A[N] = D .^ 2 * pi / 4;
  SI.Area A_[N + 1] = D_ .^ 2 * pi / 4;
  SI.Area A_m[N - 2];
  SI.Area A_m_end;
  SI.Area A_m_first;
...
equation
  ...
  for i in 1:N - 2 loop
    F_eps_m[i] = -Functions.DarcyFriction.Friction(v_exp[i + 1], 2 * sqrt(A_m[i] / pi), dx, rho_m[i], data.mu, data.p_eps) / v_exp[i + 1];
    Ap_m[1, i] = -((m_exp[i] - m_exp[i + 2]) / 4 - F_eps_m[i]);
