...
initial equation
  ...
  Vdot_p = 19.077;
equation
  para.rho * L_i * der(Vdot_i) = A_i * (p_r - p_n) + para.rho * L_i * A_i * para.g * cos_theta_i - 1 / 8 * pi * L_i * fD_i * para.rho * B_i * Vdot_i * abs(Vdot_i);
  para.rho * A_s / cos_theta_s * der(h_s) = para.rho * Vdot_s;
  para.rho / cos_theta_s * der(h_s * Vdot_s) = para.rho * Vdot_s ^ 2 / A_s + A_s * (p_n - para.p_a) - para.rho * A_s * h_s * para.g - 1 / 8 * pi * fD_s * para.rho * l_s * B_s * Vdot_s * abs(Vdot_s);
  para.rho * L_p * der(Vdot_p) = A_p * (p_n - p_tr1) + para.rho * L_p * A_p * para.g * cos_theta_p - 1 / 8 * pi * fD_p * para.rho * L_p * B_p * Vdot_p * abs(Vdot_p);
  Vdot_p = C_v * u_v * sqrt((p_tr1 - p_tr2) / para.p_a);
  //p_tr1 - p_tr2 = Vdot_p^2 * para.p_a/(C_v*u_v)^ 2;
  para.rho * L_d * der(Vdot_p) = A_d * (p_tr2 - p_t) + para.rho * L_d * A_d * para.g * cos_theta_d - 1 / 8 * pi * fD_d * para.rho * L_d * B_d * Vdot_p * abs(Vdot_p);
end HPModelDEA;
