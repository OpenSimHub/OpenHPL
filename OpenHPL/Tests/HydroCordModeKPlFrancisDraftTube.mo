...
model HydroCordModeKPlFrancisDraftTube "Model of HP system with simplified models for penstock, turbine, etc."
  ...
  OpenHPL.Waterway.Pipe discharge1(
    ...
    H=3.5,
    L=601,
    p_eps=0.075) annotation (Placement(transformation(extent={{44,-24},{64,-4}})));
  ...
  OpenHPL.Waterway.SurgeTank surgeTank(
    ...
    p_eps=0.1) annotation (Placement(transformation(
        origin={-4,66},
        extent={{-10,-10},{10,10}})));
  inner OpenHPL.Data data(Vdot_0=0.0058466) annotation (Placement(transformation(
        origin={-86,92},
        extent={{-10,-10},{10,10}})));
  ...
  OpenHPL.ElectroMech.Turbines.Francis Francis(
    ...
    r_v_=1.3,
    u_end_=2.44,
    u_start_=2.28832) annotation (Placement(transformation(
        origin={32,0},
        extent={{-10,-10},{10,10}})));
  ...
  Modelica.Blocks.Sources.CombiTimeTable rotation(columns = {2}, fileName = "C:/Users/liubomyr/OneDrive/Documents/PhD/HydroCord/Rotation.txt", tableName = "rotation", tableOnFile = true) annotation (
    Placement(transformation(origin = {-80, -44}, extent = {{-10, -10}, {10, 10}})));
  ...
  Modelica.Blocks.Nonlinear.Limiter limiter1(uMax=100, uMin=0.01) annotation (Placement(transformation(
        origin={-50,-70},
        extent={{-10,-10},{10,10}})));
  ...
  OpenHPL.Waterway.Pipe draftTube(
    ...
    H=12,
    L=draftTube.H,
    p_eps=0.001) annotation (Placement(transformation(extent={{44,4},{64,24}})));
equation
  ...
  connect(fitting1.n, penstock2.p) annotation (
    Line(points={{14,20},{17.9,20},{17.9,4},{-2,4}}, color = {28, 108, 200}));
  connect(penstock1.n, fitting1.p) annotation (
    Line(points={{28,52},{13.9,52},{13.9,40},{14,40}}, color = {28, 108, 200}));
  connect(surgeTank.n, penstock1.p) annotation (
    Line(points={{6,66},{8,66},{8,52}}, color = {28, 108, 200}));
...
end HydroCordModeKPlFrancisDraftTube;
