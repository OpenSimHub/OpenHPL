...
equation
  ...
  K_pd = Z_p * K_dp / Z_d;
  K_pdz = Z_p * K_dp;
  p_n = K_p / Z_i * (Z_i * p_r + Z_s * data.p_a + Z_p * K_dp * (Vdot_p ^ 2 * data.p_a / (C_v ^ 2 * u ^ 2) + p_t) + data.rho * data.g * (A_p * cos_theta_p * (K_pd - 1) - K_pd * A_d * cos_theta_d + A_s * cos_theta_s + A_i * cos_theta_i) + 1 / 8 * pi * data.rho * (fD_s * B_s * Vdot_s * abs(Vdot_s) + (fD_d * B_d * K_pd + fD_p * B_p * (1 - K_pd)) * Vdot_p * abs(Vdot_p) - fD_i * B_i * Vdot_i * abs(Vdot_i)));
  p_tr1 = K_z * (Z_p * p_n + Z_d * (Vdot_p ^ 2 * data.p_a / (C_v ^ 2 * u ^ 2) + p_t) + data.rho * data.g * (A_p * cos_theta_p - A_d * cos_theta_d) + 1 / 8 * pi * data.rho * Vdot_p * abs(Vdot_p) * (fD_d * B_d - fD_p * B_p));
  der(h_s) = Vdot_s * cos_theta_s / A_s;
  der(Vdot_s) = Z_s / data.rho * (p_n - data.p_a) - A_s * data.g * cos_theta_s - 1 / 8 * pi * fD_s * B_s * Vdot_s * abs(Vdot_s);
  der(Vdot_p) = Z_p / data.rho * (p_n - p_tr1) + A_p * data.g * cos_theta_p - 1 / 8 * pi * fD_p * B_p * Vdot_p * abs(Vdot_p);
end HPModel;
