...
annotation (
    Documentation(info = "<html><head></head><body><p>Simplified empirical turbine model for single-regulated reaction turbine (Francis and propeller turbine). The turbine is specified by giving the nominal head <b>H_n</b> and nominale power <b>P_n</b>.<br/>All remaining values are determined from empirical relations. The throtling effect of high head Francis turbines is included in the model. However, the exact characteristics should be treated with caution and will need more empirical tuning in future releases.
