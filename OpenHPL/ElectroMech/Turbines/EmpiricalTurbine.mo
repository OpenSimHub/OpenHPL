...
model EmpiricalTurbine
 ...
 protected
  Modelica.Blocks.Sources.RealExpression realExpression(y=Tt) "Connection between compute hydraulic torque and  driving torque in class TorqueEquation" annotation (Placement(transformation(extent={{-70,-10},{-50,10}})));
...
equation
  ...
  //
  if enable_P_out then
    P_out=Tt*speedSensor.w;
  end if;
