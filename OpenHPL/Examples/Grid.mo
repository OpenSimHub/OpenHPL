...
<p>The given grid has a bias factor of &lambda;=266.6&nbsp;MW/Hz.
The <code>simpleGen</code> is generating 100&nbsp;MW in order to supply the the
internal grid load of 100&nbsp;MW. The system is in balance.<p>
<p>At <code>t=10s</code> the grid loses 133.3 MW of production 
(see <code>loadStep</parameters>) and now two different cases can be investigated:</p>
<h5>No self-regulation</h5>
<p>When simulating the system with the setting of <code>grid.mu=0</code> one should expect 
the frequency difference &Delta;F (see <code>grid.dF.y</code>) to settle down at
&Delta;f = -0.5 Hz.</p>
<h5>With self-regulation</h5>
<p>Changing the setting of the self-regulation to <code>grid.mu=2</code> should now 
result in a frequency difference &Delta;f (see <code>grid.dF.y</code>) that settles down at
a slighly lower &Delta;f &approx; -0.496 Hz.</p>
<p>Note: When doing the calculation based by hand and calculating the new &lambda; which includes 
