...
function Tapered "Calculation of phi for Tapered reduction/expansion"
  ...
  input SI.Conversions.NonSIunits.Angle_deg theta "Angle of taper";
...
algorithm
  if D_o <= D_i then
    /* Tapered Reduction */
    if theta < 22.5 then
      phi := 1.6*sin(SI.Conversions.from_deg(theta)/4)
             *Square(N_Re,p_eps,D_i,D_o);
    else
      phi := sqrt(sin(SI.Conversions.from_deg(theta)/4))
             *Square(N_Re,p_eps,D_i,D_o);
    end if;
  else
    /* Tapered Expansion */
    if theta < 22.5 then
      phi := 2.6*sin(SI.Conversions.from_deg(theta)/4)
             *Square(N_Re,p_eps,D_i,D_o);
    else
